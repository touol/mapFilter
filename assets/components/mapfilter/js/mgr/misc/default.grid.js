mapFilter.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        url: mapFilter.config['connector_url'],
        baseParams: {},
        cls: config['cls'] || 'main-wrapper mapfilter-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('mapfilter-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('mapfilter-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('mapfilter-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('mapfilter-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    mapFilter.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(mapFilter.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = mapFilter.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('mapfilter-grid-default', mapFilter.grid.Default);