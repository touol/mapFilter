...
mapFilter.utils.formatDate = function (string) {
    if (string && string != '0000-00-00 00:00:00' && string != '-1-11-30 00:00:00' && string != 0) {
        ...
        return date.strftime(mapFilter.config['date_format']);
    }
    else {
        return '&nbsp;';
    }
...
mapFilter.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

mapFilter.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
mapFilter.utils.renderActions = function (value, props, row) {
    var res = [];
    ...
    for (var i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="mapfilter-btn mapfilter-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="mapfilter-row-actions">{0}</ul>',
        res.join('')
    );
};


mapFilter.utils.userLink = function (value, id, blank) {
    if (!value) {
        return '';
    }
    else if (!id) {
